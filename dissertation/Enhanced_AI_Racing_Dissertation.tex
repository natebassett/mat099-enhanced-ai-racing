...
\begin{figure}[htbp]
  ...
  \caption{Agent-environment loop and explanation layer in Enhanced AI Racing.}
...
The results answer two related questions: whether the implemented agents can complete the common racing task, and whether the application makes the resulting evidence inspectable. \Cref{tab:agent-results} combines application database records with repeated fixed-policy evaluations. The sources are shown separately because they support different claims. Database records demonstrate integrated application behaviour; repeated batches provide stronger evidence of the reliability of final learned checkpoints. All use the common \texttt{g-track-3}, car and one-lap scenario described in \cref{ch:testing}.
...
\Cref{fig:completion-reliability} shows completion reliability, with each bar labelled by completed and attempted laps rather than percentage alone. The engineered agents provide an important benchmark. The Map-Aware Racing-Line Agent completed all 15 stored application attempts, recorded no off-track events and achieved a 92.056-second median completed lap. It therefore demonstrates that explicit route knowledge and hand-engineered control can be both reliable and fast on the known circuit. The Rule-Based Anti-Spin Agent also completed both stored attempts. Its small sample makes 100\% a weak estimate of general reliability, but it remains useful evidence of stable baseline behaviour.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.90\textwidth]{figure_10_1_completion_reliability.png}
  \caption{Completion reliability across the recorded evaluation batches. Bar labels retain completed and attempted laps, so percentage values are not interpreted without their denominators. Source: project evaluation records.}
  \label{fig:completion-reliability}
\end{figure}

The early learning results prevent an overly simple narrative in which more complex algorithms automatically win. Dyna-Q completed one of three stored attempts, and its only completed lap took 305.562 seconds. Scratch TD3 completed none of six stored attempts. These are not failed dissertation outcomes. They identify the limits of a coarse tabular state-action design and an immature continuous-control training setup. In particular, they show why the project moved from local reward improvement towards repeated completion as the decisive criterion for later policy selection.

\subsection{Learned-policy performance}

\Cref{fig:lap-time-comparison} compares fastest and median completed-lap times, while \cref{fig:reliability-speed-tradeoff} plots completion rate against median completed-lap time. Agent~7 established that N-step TD3 could produce repeated learned completion. Version~3 completed 18 of 20 attempts with a 104.220-second median; version~4 completed 10 of 10 in its smaller batch with a 103.887-second median. The results are reliable within their recorded batches but slower than the map-aware baseline.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.90\textwidth]{figure_10_2_lap_time_comparison.png}
  \caption{Fastest and median completed-lap time by agent. Times are conditional on lap completion; reliability is shown separately in \cref{fig:completion-reliability}. Source: project evaluation records.}
  \label{fig:lap-time-comparison}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.90\textwidth]{figure_10_3_reliability_speed_tradeoff.png}
  \caption{Observed trade-off between completion rate and median completed-lap time. The evidence source and attempt count for each agent remain available in \cref{tab:agent-results}. Source: project evaluation records.}
  \label{fig:reliability-speed-tradeoff}
\end{figure}

Agent~8 produced the strongest observed final learned result. Across two 20-run repeated-evaluation batches, it completed 17 and 19 attempts respectively, giving 36 completions from 40 attempts. Its median and best completed-lap time were both 83.038 seconds. The equality of best and median indicates tightly clustered completed outcomes in the retained records, but it must not conceal the four failed attempts. Agent~8 is therefore not described as perfect; it is the fastest observed final policy while achieving 90\% repeated completion.

The comparison between Agent~7 and Agent~8 is informative but bounded. Agent~7 uses racing-line geometry as context, whereas Agent~8 uses sensors and history without explicit route data. The faster Agent~8 outcome suggests that a sensor-only policy can discover an effective local driving strategy on \texttt{g-track-3}. It does not prove that racing-line information is unhelpful, because the final agents also differ in reward and training history. The map-aware agent remains the better source of direct, human-readable route explanation, while Agent~8 provides the stronger learned pace result.

\subsection{Learning progression and result reliability}

\Cref{fig:td3-evaluation-progression} shows the evaluation progression of the TD3 work. Its purpose is not decoration; it demonstrates why the final policy was not selected from a single exceptional event. Early evaluations frequently failed to complete a lap. Agent~7 improved from non-completion to repeated successful policy evaluation. Agent~8 then developed from rare fast laps towards higher clean completion through stability-focused training and self-imitation of its own successful trajectories.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.90\textwidth]{figure_10_4_td3_evaluation_progression.png}
  \caption{Progression of the TD3 evaluation work, showing why the final policy was selected from repeated outcomes rather than a single exceptional lap. Source: project evaluation records.}
  \label{fig:td3-evaluation-progression}
\end{figure}

\Cref{fig:repeated-evaluation-outcomes,fig:lap-time-distribution} make this evidence auditable. The repeated-outcome chart displays successful and failed trial counts for each final batch. The lap-time distribution shows only completed laps and therefore has a conditional interpretation. The Map-Aware agent's completed laps cluster in the low 90-second range, Agent~7's around 104 seconds and Agent~8's around 83 seconds. Taken together, these figures show a trade-off rather than a simplistic ranking: engineered control was transparent and highly stable, Agent~7 was a reliable learned policy with route context, and Agent~8 achieved the strongest observed pace-reliability balance.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.90\textwidth]{figure_10_5_repeated_evaluation_outcomes.png}
  \caption{Successful and failed outcomes in the final repeated evaluation batches. Source: project evaluation records.}
  \label{fig:repeated-evaluation-outcomes}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.90\textwidth]{figure_10_6_lap_time_distribution.png}
  \caption{Distribution of completed-lap times. Failed attempts are not assigned a lap time and remain represented in the completion evidence. Source: project evaluation records.}
  \label{fig:lap-time-distribution}
\end{figure}

\subsection{Application result}

The software result is that this evidence is not confined to scripts and model folders. The packaged application satisfies the functional workflow conditions in \cref{tab:novice-acceptance}: a user can select an agent, start TORCS, inspect live telemetry, review a stored run and compare agents through one interface. Failure is made informative rather than hidden: a termination label and telemetry trace can show whether a run stalled, moved in the wrong direction or left the circuit, while the comparison view places that outcome beside successful laps.

This directly addresses the novice-facing learning objective. The interface is designed to let an AI newcomer observe why a single fast lap is insufficient and compare the consequences of rules, track information and learned policies without reading the codebase. The result is functional evidence of accessible deployment and explanation, supported by the workflow evidence in \cref{tab:novice-acceptance} and the screens in \cref{fig:live-dashboard,fig:run-review}. It is not a claim that the interface has already been proven to teach reinforcement learning more effectively than another presentation method.
